...
{
#pragma region 삽입 정렬
	// 두 번째 자료부터 시작하여 그 앞(왼쪽의) 자료들과 비교하여 삽입할 위치를 지정한 후 자료를 뒤로 옮기고 지정한 자리에 자료를 삽입하여 정렬하는 알고리즘 입니다.
...
#pragma region 시간 복잡도
	// 컴퓨터 프로그램의 입력 값과 연산 수행 시간의 상관관계를 나타내는 척도입니다.

	// Big-O 표기법
	// 입력값의 변화에 따라 연산을 수행할 때, 연산 횟수에 비해 시간이 얼마만큼 걸리는 지 나타내는 척도입니다.
	...
	// O(n) 선형 시간 복잡도
	// 입력값이 증가함에 따라 시간 또한 같은 비율로 증가하는 시간 복잡도입니다.
